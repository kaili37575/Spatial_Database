...
String query = "select * from killer_tomato";	
ResultSet  rs;
int scale=2;
int [] ia3;
int [] ia4;
	...
	//add ten function button         
			...
			union.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
			    	...
			    	query = "select sdo_geom.sdo_union(k1.shape,k2.shape,0.5) shape from killer_tomato k1,killer_tomato k2 where k1.part_name='"+in1+"' AND k2.part_name='"+in2+"'";	
			...
			intersect.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String in1=input1.getText();
					String in2=input2.getText();
			    	query = "select sdo_geom.sdo_intersection(k1.shape,k2.shape,0.5) shape from killer_tomato k1,killer_tomato k2 where k1.part_name='"+in1+"' AND k2.part_name='"+in2+"'";	
			...
			difference.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String in1=input1.getText();
					String in2=input2.getText();
			    	query = "select sdo_geom.sdo_difference(k1.shape,k2.shape,0.5) shape from killer_tomato k1,killer_tomato k2 where k1.part_name='"+in1+"' AND k2.part_name='"+in2+"'";	
			...
			convexhull.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
			    	...
			    	query = "select sdo_geom.sdo_convexhull(k1.shape,0.5) shape from killer_tomato k1 where k1.part_name='"+in1+"'";	
			...
			mbr.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
			    	...
			    	query = "select sdo_geom.sdo_mbr(k1.shape) shape from killer_tomato k1 where k1.part_name='"+in1+"'";	
			...
			distance.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String in1=input1.getText();
					String in2=input2.getText();
			    	query = "select sdo_geom.sdo_distance(k1.shape,k2.shape,0.5) shape from killer_tomato k1,killer_tomato k2 where k1.part_name='"+in1+"' AND k2.part_name='"+in2+"'";	
			...
			area.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
			    	...
			    	query = "select sdo_geom.sdo_area(k1.shape,0.5) shape from killer_tomato k1 where k1.part_name='"+in1+"'";	
			...
			relate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String in1=input1.getText();
					String in2=input2.getText();
			    	query = "select sdo_geom.relate(k1.shape,'contains',k2.shape,0.5) shape from killer_tomato k1,killer  _tomato k2 where k1.part_name='"+in1+"' AND k2.part_name='"+in2+"'";	
			...
			centroid.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String in1=input1.getText();
					//String in2=input2.getText();
			    	query = "select sdo_geom.sdo_centroid(k1.shape,0.5) shape from killer_tomato k1 where k1.part_name='"+in1+"'";	
			...
			length.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					String in1=input1.getText();
					//String in2=input2.getText();
			    	query = "select sdo_geom.sdo_length(k1.shape,0.5) shape from killer_tomato k1 where k1.part_name='"+in1+"'";	
			...
			insert.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
			    	...
			    	query = "INSERT INTO killer_tomato VALUES ( 40,'bird2',MDSYS.SDO_GEOMETRY (2003,null,null,MDSYS.SDO_ELEM_INFO_ARRAY(1,2,2), MDSYS.SDO_ORDINATE_ARRAY(90,260,75,270,60,260,45,270, 30,260)))";	
			    	flag=1;
			    	System.out.println(query);
                    disp1.repaint();
...
//INSERT INTO killer_tomato VALUES ( 13,'humman_body',MDSYS.SDO_GEOMETRY (2003,null,null,MDSYS.SDO_ELEM_INFO_ARRAY(1,1003,3),MDSYS.SDO_ORDINATE_ARRAY(280,100,330,160)))		
			
			delete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					System.out.println("DELETE");
					String in3=input3.getText();
			    	query =" delete from killer_tomato where PART_NAME='bird2'";
			    	
			    	flag=1;
			    	System.out.println(query);
                    disp1.repaint();
 //delete from killer_tomato where PART_NAME='humman_body'                 
				}
			...
			result.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					flag=0;
					query = "select * from killer_tomato";	
					disp1.repaint();
		//			JOptionPane.showMessageDialog(null,"Answer is :"+s2,"Result Answer",JOptionPane.INFORMATION_MESSAGE);
				}
	...
	public class dspimg extends JPanel {	  
		...
		protected void paintComponent(Graphics g) { //override paint 
			...
			super.paintComponent(g);
			 ...
			 String url;
			    url = "jdbc:oracle:thin:@localhost:1522:orcl";// my oracle database port is 1522
			    Statement stmt;
			    Connection conn;
			    try {   // loads drivers
			    ...
			    try {
			      ...
			      conn = DriverManager.getConnection(url,"kai","apple"); //my database user and password  
						      ...
						      return;
						       ...
						       // System.out.println(len4);
						    	...
						    	if(ia3[1]==4){//Compound line
						    		...
						    		inxx=inxx+3;//set inital index=3,just skip first 3 number
						    		temp=1;//this is a flag, if it's Compound polygon, when changing draw type, there is a point which we need to use it twice
...
//calculate the origin point and radius	of the circle	
	    ...
	    x0=(int)Math.round(((y2*y2+x2*x2-x1*x1-y1*y1)*(y3-y2)-(x3*x3+y3*y3-x2*x2-y2*y2)*(y2-y1))/2/((y2-y1)*(x2-x3)-(y3-y2)*(x1-x2)));
	    y0=(int)Math.round(((x1*x1+y1*y1-x2*x2-y2*y2)*(x2-x3)-(x2*x2+y2*y2-x3*x3-y3*y3)*(x1-x2))/2/((y3-y2)*(x1-x2)-(y2-y1)*(x2-x3)));
	   ...
	   double degree_offset1=(360* ((Math.asin(0.5*d1/r))/Math.PI));
	   double degree_offset2=(360* ((Math.asin(0.5*d2/r))/Math.PI));
	   int degree_offset=(int) Math.round((degree_offset1+degree_offset2));
	    //counter clockwise angle offset
	    int thet1=(int) Math.round(Math.toDegrees(Math.acos((x3-x0)/r)));//using arccos to calculate 1st,2nd quadrant are whole peroid
	    

	   // System.out.println(thet1);
	    //System.out.println(degree_offset);

	    
	   int xx1=(int)Math.round(x0-r);
	   int yy1=(int)Math.round(y0+r);
if(y3-y0<0){
	    	    g2.drawArc(xx1, -yy1, r*2, r*2, 360-thet1, -degree_offset);// start point in 3rd or 4th quadrant
}else{

	g2.drawArc(xx1, -yy1, r*2, r*2, thet1,-degree_offset);// start point in 1st or 2nd quadrant
}
 x1=ia4[i+2];
 y1=ia4[i+3];
